...
START = find(lambda i: LINES[i].strip() == "<The Celebration and", PAST + 1)
END = find(lambda i: LINES[i].strip() == "<A Thanksgiving for the Birth", START + 1)
...
def render(block, level):
    ...
    # damaged source: an italic rubric opened with '*' but never closed (the e-text
    # dropped the closing '*' and some words); strip the stray leading '*', keep the
    # rest verbatim, and let the caller flag it.
    return [re.sub(r"^\*", "", deitalic(joined).strip())]
...
body = transform(START, END, ["The Celebration and"])

# The PD e-text is damaged in two spots; keep it verbatim and flag each inline so a
# scan reconciles it (never invent the missing words). verify_index keys on the
# FIRST single-quoted reading in each comment.
VERIFY = {
