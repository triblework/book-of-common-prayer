...
START = find(lambda i: LINES[i].strip() == "<Ministration to the Sick>", PAST + 1)
END = find(lambda i: LINES[i].strip() == "<Ministration at the", START + 1)
...
body = transform(START, END, ["Ministration to the Sick"])
