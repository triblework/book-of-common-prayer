...
Tier-1 Daily Evening Prayer office file, mechanically. The liturgical text flows
source -> here -> file; it is never re-typed. Rite I is the office body; Rite II
is a clearly separated `## Rite Two` section (spec §4.2).

This is the Evening-Prayer sibling of ingest/transform_1979.py; only the section
boundaries and the document framing differ.
"""
import sys, re
...
#   Rite One Evening Prayer 2751 .. (ends at Rite Two Morning Prayer heading)
#   Rite Two Evening Prayer 5073 .. (ends at An Order for Compline)
R1_EP = find(lambda i: two_line(i, "<Daily Evening Prayer:", "Rite One>"))
R1_END = find(lambda i: two_line(i, "<Daily Morning Prayer:", "Rite Two>"), R1_EP+2)
R2_EP = find(lambda i: two_line(i, "<Daily Evening Prayer:", "Rite Two>"))
R2_END = find(lambda i: LINES[i].strip().startswith("<An Order for Compline"), R2_EP+2)
assert 0 <= R1_EP < R1_END and 0 <= R2_EP < R2_END, (R1_EP, R1_END, R2_EP, R2_END)
...
def unwrap_headings(seg):
    """Join a heading that wraps across two lines ('<Daily Evening Prayer:' / 'Rite One>')
...
# Obvious heading typos in the e-text, corrected so the cross-edition anchors align.
# Both are confirmed by the accompanying canticle citation / by the Rite Two heading:
#   'The Song of Solomon' is labelled '=Nunc dimittis=' (the Song of SIMEON; Rite Two
#       prints '<Song of Simeon>' correctly) -> a plain e-text typo (Solomon->Simeon).
#   'The Apostle's Creed' -> 'The Apostles' Creed' (Rite Two prints it correctly).
HEADING_FIXES = {
    "The Song of Solomon": "The Song of Simeon",
    "The Apostle's Creed": "The Apostles' Creed",
}
...
def render(block, level):
    ...
    if first.startswith("<") and ">" in first:
        ...
        title = HEADING_FIXES.get(title, title)
...
def transform(i0, i1, level):
    ...
    for blk in blocks(seg):
        if blk[0].strip().startswith("<Daily Evening Prayer"):
            continue
...
rite1 = transform(R1_EP, R1_END, "##")
rite2 = transform(R2_EP, R2_END, "###")
...
txt = "\n".join(doc)
print("boundaries R1: %d-%d  R2: %d-%d" % (R1_EP, R1_END, R2_EP, R2_END))
